...
{
    // A list of tags which we use to determine whether to destroy or not
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (tagListType == TagListType.Blacklist 
            && tagInList)
        {
            // Process Collision if it's a Blacklist and the tag IS in the Blacklist
            ProcessCollision(other.gameObject);
        }
        else if (tagListType == TagListType.Whitelist 
            && !tagInList)
        {
            // Process Collision if it's a Whitelist and the tag is NOT in the Whitelist
            ProcessCollision(other.gameObject);
        }
    }
